...
- 32 个模型、34 个数据集资源规格。
- 13 个专用医学 benchmark、2 个通用 benchmark 和 1 个组合 runner。
- 13 套固定的任务/标注/提示/评分协议；34 个数据条目不计作 34 个完整评测。
...
| v1.6 | 13 个专用医学 benchmark，Slurm 作业 437789 | 21 个合成样本逐项完成 audit、推理、专用评分和报告；不是质量分数 |
...
- 多节点后端和长时间稳定性压测。
- 专家标注报告生成、定位和测量真实切片。
- 13 个专用 benchmark 仍需逐项补齐固定 revision 的真实模型和真实数据运行证据。
- 临床安全偏好数据上的真实后训练。
